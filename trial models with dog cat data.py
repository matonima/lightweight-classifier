# -*- coding: utf-8 -*-
"""
Created on Tue Nov 24 16:06:50 2020
...
traindata = train_datagen.flow(x=x_train, y=y_train, batch_size=32, shuffle=True)

valdata = test_datagen.flow(x=x_test, y=y_test, batch_size=32, shuffle=True)
...
traindata = trdata.flow_from_directory(directory="train",target_size=(112,112),batch_size=2, shuffle=True, subset=('training'), class_mode='categorical',color_mode="grayscale")
valdata = trdata.flow_from_directory(directory="train",target_size=(112,112), batch_size=2,shuffle=True, subset=('validation'), class_mode='categorical',color_mode="grayscale")
tsdata = ImageDataGenerator()
# testdata = tsdata.flow_from_directory(directory="test", shuffle= False, target_size=(224,224), class_mode='categorical')
testdata = tsdata.flow_from_directory(directory="test",batch_size=1476, shuffle=True, target_size=(112,112), class_mode='categorical',color_mode="grayscale")
class_names=["cat", "dog"]
...
def model_builder(hp):
    act='tanh'
    hp_units=hp.Int('units', min_value = 30, max_value = 60, step = 4)#optimal value from min to max is chosen
    hp_units2=hp.Int('units', min_value = 20, max_value = 40, step = 2)#optimal value from min to max is chosen
...
tuner.search(x=traindata,validation_data=valdata,verbose=1,epochs=10,steps_per_epoch=10)
#hist=model_builder.fit(traindata, epochs = 1, verbose =1, validation_data=valdata, steps_per_epoch=50)
print('Model built')
...
hist2 = best_model2.fit(x = traindata, epochs =100, verbose =1, validation_data=valdata,steps_per_epoch = 25)
...
prediction1=np.zeros((len(data),2))
prediction2=np.zeros((len(data),2))
for i in range(len(data)):
   image = data[i].reshape(1,112,112,1)
   #image = image/255
